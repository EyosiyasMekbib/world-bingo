...
const getGameUrl = vi.fn(async (p: any) => ({
  gameUrl: 'https://play/' + p.username,
  token: 'tok',
}))
...
beforeEach(() => {
  ...
  process.env.HUB_DEPLOYMENTS = JSON.stringify([
    { code: 's01', baseUrl: 'https://s01', secret: SECRET },
  ])
...
describe('internal provider API', () => {
  it('namespaces the account for a launch from s01', async () => {
    ...
    expect(getGameUrl).toHaveBeenCalledWith(
      expect.objectContaining({ username: 's01' + 'a'.repeat(32) }),
    )
  })
  ...
  it('proxies a positional catalog method without namespacing', async () => {
    ...
    const res = await post(app, {
      providerCode: 'palace',
      method: 'getGames',
      params: { args: ['vendorX', 1, 60, 'ETB', 'en'] },
    })
    expect(res.statusCode).toBe(200)
  ...
  it('rejects an unknown / unsigned deployment with 401', async () => {
    ...
    const res = await post(
      app,
      { providerCode: 'palace', method: 'getGameUrl', params: {} },
      'wrong-secret',
    )
  ...
  it('returns ok:false when username is missing for a user-scoped method', async () => {
    ...
    expect(res.json()).toMatchObject({
      ok: false,
      error: { message: 'params.username is required' },
    })
  ...
  it('returns ok:false for an unknown method', async () => {
    ...
    expect(res.json()).toMatchObject({
      ok: false,
      error: { message: 'unknown method: noSuchMethod' },
    })
  })

  it('returns provider errors in the body (ok:false), not as a throw', async () => {
    getGameUrl.mockRejectedValueOnce(
      Object.assign(new Error('boom'), { code: 'PALACE_X', palaceCode: 42 }),
    )
    const app = await buildApp()
    const res = await post(app, {
      providerCode: 'palace',
      method: 'getGameUrl',
      params: { username: 'c'.repeat(32) },
    })
    expect(res.statusCode).toBe(200)
    expect(res.json()).toMatchObject({
      ok: false,
      error: { message: 'boom', code: 'PALACE_X', palaceCode: 42 },
    })
  })
})

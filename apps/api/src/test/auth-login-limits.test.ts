...
// Password sign-in was keyed per IP at 10/min. Behind an Ethiopian carrier NAT
// that is one budget for a whole neighbourhood, so a busy evening throttles
// honest callers. Key on the identifier being tried instead; an IP ceiling on
// the route keeps credential stuffing from minting unlimited buckets. Since
// players moved to Firebase phone sign-in, /auth/admin/login is the only

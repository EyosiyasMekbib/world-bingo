...
/**
 * `Kebede Mekonnen` -> `Kebede Mekonnen`
 *
 * The full name, deliberately. An agent has the player in front of them and
 * has to be sure the cash and the account match before they confirm, and an
 * initial is not enough to do that out loud. The trade is that a fulfilled
 * code tells the agent who the player is; the code is short-lived and only
 * the agent who takes the cash ever sees this.
 *
 * Falls back to the username when no real name is on the account, and to
 * `Player` when there is nothing at all, so the panel never renders blank.
 */
function playerDisplayName(
  ...
  const full = [(user.firstName ?? '').trim(), (user.lastName ?? '').trim()]
    .filter(Boolean)
    .join(' ')
  return full || (user.username ?? '').trim() || 'Player'
}

/**
 * `+251912345284` -> `•••• 5284`
 *
 * The last four digits only. That is what a player can read back at a counter
 * to prove the code is theirs, and it is still not a number the agent could
 * dial or reuse. The name beside it carries the identification; this is the
 * confirming detail.
 *
 * Null when there is no phone, or fewer than four digits to show.
 */
...
export class AgentDepositService {
  ...
  static async lookupByCode(code: string) {
    ...
    return {
      ...
      player: {
        name: playerDisplayName(user),
        phoneTail: maskPhone(user?.phone),
  ...
  static async fulfill(code: string, agentUserId: string) {
    ...
    const committed = await prisma.$transaction(async (tx) => {
      ...
      const agents = await tx.$queryRaw<
        Array<{ id: string; userId: string; float: Decimal; accountStatus: string }>
      >`

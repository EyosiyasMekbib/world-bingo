...
<template>
  <div class="min-h-screen flex flex-col" style="background: var(--color-background, #0a0d14); color: var(--color-text-primary, #fff);">
      ...
      <div class="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        ...
        <nav class="hidden sm:flex items-center gap-1">
          ...
          <NuxtLink
            ...
            active-class="text-amber-400 bg-amber-400/10"
          >
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" /></svg>
            {{ t('nav.refer') }}
          </NuxtLink>
          <NuxtLink
            to="/tournaments"
          ...
          <NuxtLink
            ...
            active-class="text-amber-400 bg-amber-400/10"
          >
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
            Profile
          </NuxtLink>
        </nav>
    ...
    <DepositModal
      v-model="showDeposit"
      @deposited="(auth as any).fetchWallet(); showDeposit = false"
    />
    <WithdrawalModal
      v-model="showWithdrawal"
      :balance="Number(auth.wallet?.balance ?? 0)"
      @withdrawn="(auth as any).fetchWallet(); showWithdrawal = false"
    />
  </div>
</template>
